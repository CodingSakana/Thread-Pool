...
auto ThreadPool::enqueue(F&& f, Args&&... args) -> std::future<std::invoke_result_t<F, Args...>> {
    ...
    // 创建一个 shared_ptr 来管理 std::packaged_task，保证可拷贝性
    auto task_ptr =
        std::make_shared<std::packaged_task<return_type()>>(std::packaged_task<return_type()>(
            ...
            //
            // mutable 允许 lymbda 表达式修改捕获的变量（默认是 const 模式，不可修改）
            [func = std::forward<F>(f), ... params = std::forward<Args>(args)]() mutable {
                // 在工作线程中调用实际的可调用对象并返回结果
                return func(params...);
            }));
